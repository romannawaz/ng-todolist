...
export class ListComponent {
  ...
  private sortUsers(users: User[]): User[] {
    if (this.sortSig()[UserFields.NAME] !== undefined) {
      users.sort((a, b) =>
        this.sortSig()[UserFields.NAME]
          ? a[UserFields.NAME].localeCompare(b[UserFields.NAME])
          : b[UserFields.NAME].localeCompare(a[UserFields.NAME]),
      );
    }

    if (this.sortSig()[UserFields.AGE] !== undefined) {
      users.sort((a, b) =>
        this.sortSig()[UserFields.AGE]
          ? a[UserFields.AGE] - b[UserFields.AGE]
          : b[UserFields.AGE] - a[UserFields.AGE],
      );
    }
